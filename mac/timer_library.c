...
int __getRealTime(struct timespec *ts) {
...
double getResolutionOfTimer ( int timerType )
{
	...
	switch ( timerType )
	{
		...
		struct timespec start_realtime, end_realtime;
            ...
            if ( __getRealTime(&start_realtime) != 0) {
			logErrorMessage ( \
	        ...
	        do {
                ...
                if (__getRealTime(&end_realtime) != 0) {
				logErrorMessage ( \
			      TIMER_LIB_CLOCK_GETTTIME_REALTIME_RETURNED_ERROR);
				return MAXDOUBLE;
                }
...
******************************************************************************/
JNIEXPORT void JNICALL
Java_highrestimer_HighResTimer_diff(JNIEnv *env, jobject this)
{
	double stop = 0.0;
	double start = 0.0;
	jfieldID fid;
...
int getTimeStamp(double *timeStamp) 
{
	*timeStamp = MAXDOUBLE;
	...
	case TIMER_LIB_USE_REALTIME : {
        ...
        if ( __getRealTime(&realtime_timestamp) != 0) {
			logErrorMessage ( \
			      TIMER_LIB_CLOCK_GETTTIME_REALTIME_RETURNED_ERROR);
